document.addEventListener("DOMContentLoaded", () => {
  ...
  // ==========================================
  // AI DSA Mentor Chat Drawer Logic
  // ==========================================
  ...
  document.addEventListener("click", (e) => {
    if (chatDrawer && chatDrawer.classList.contains("open")) {
      if (!chatDrawer.contains(e.target) && !chatToggleBtn.contains(e.target)) {
        chatDrawer.classList.remove("open");
      }
    }
  });
  ...
  function formatMessageMarkdown(text) {
    ...
    // Escape HTML tags to prevent syntax injections
    let escaped = text
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;");
    
    // Replace triple backtick blocks: ```language [code] ```
    escaped = escaped.replace(/```(?:[a-zA-Z]*)\n?([\s\S]*?)\n?```/g, (match, codePart) => {
      return `<pre><code>${codePart.trim()}</code></pre>`;
    });
    
    // Replace inline backticks: `code`
    ...
    return paragraphs
      .map(p => {
        // If it starts with <pre>, don't wrap in p tag to maintain layout
  ...
  function appendChatMessage(role, text, isHtml = false) {
    ...
    if (isHtml) {
      ...
      messageDiv.innerHTML = formatMessageMarkdown(text);
    }
  ...
  async function handleSendChatMessage(textToSend) {
    const rawMessage = textToSend || chatInput.value.trim();
    if (!rawMessage) return;

    // Clear input if sending from keyboard/form
    if (!textToSend) {
      chatInput.value = "";
      chatInput.style.height = "44px"; // reset input height
    }
    ...
    // Append typing loader placeholder bubble
    const typingBubble = appendChatMessage(
      "mentor",
    ...
    try {
      ...
      if (data.success && data.reply) {
        ...
        appendChatMessage("mentor", "Sorry, I encountered an issue analyzing the response. Let's try again.");
      }
    } catch (err) {
      console.error(err);
      if (typingBubble) typingBubble.remove();
      appendChatMessage("mentor", "⚠️ Server connection failed. Check your internet connection and try again.");
    } finally {
  ...
  if (chatInput) {
    ...
    // Auto-resize input bar height based on input length
  ...
  // Wire up Suggestion Chips clicks
  if (chatSuggestions) {
